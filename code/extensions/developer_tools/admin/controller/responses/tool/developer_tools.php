...
class ControllerResponsesToolDeveloperTools extends AController
{
    ...
    public function doCloneTemplate()
    {
        if (!$this->request->is_POST()) {
            redirect($this->html->getSecureURL('setting/setting', '&active=appearance'));
        }
        ...
        if (!$result) {
            ...
            $error->toJSONResponse(
                ...
                [
                    'error_text'  => implode('<br>', $this->model_tool_developer_tools->error),
                    'reset_value' => false
                ]
            );
            return;
    ...
    public function summary()
    {
        ...
        if (is_file(DIR_EXT . $this->data['project']['extension_txt_id'] . DS . 'image' . DS . $this->data['project']['icon'])) {
            $this->data['project']['icon_href'] = HTTPS_CATALOG . $this->data['project']['extension_txt_id']
                . DS . 'image'
                . DS . $this->data['project']['icon'];
        } else {
