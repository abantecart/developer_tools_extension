...
class ControllerPagesToolDeveloperTools extends AController
{
    ...
    {
        return (bool)$this->session->data['dev_tools_prj_id'];
    }
    ...
    public function main()
    {
        ...
        $this->data['heading_title'] = $this->language->get('developer_tools_name')
            . ': ' . $this->session->data['dev_tools_prj_id'];
        ...
        $this->addChild(
            'responses/tool/developer_tools/summary',
            'project_summary',
    ...
    public function project_list()
    {
        ...
        if ($projects) {
            ...
            $this->data['developer_tools_text_no_projects'] = sprintf(
                $this->language->get('developer_tools_text_no_projects'),
                DIR_EXT . 'developer_tools' . DS . 'projects' . DS
            );
        }
        ...
        //load tabs controller
        $tabs_obj = $this->dispatch('pages/tool/developer_tools_tabs', ['list']);
        $this->data['dev_tabs'] = $tabs_obj->dispatchGetOutput();
    ...
    public function create()
    {
            ...
            if ($result) {
                ...
                if (has_value($this->request->post['clone_method'])) {
                    $url = $this->html->getSecureURL(
                        'setting/setting',
                        '&active=appearance&tmpl_id=' . $this->request->post['extension_id']
                    );
                } else {
                ...
                $this->data['error_warning'] = $this->language->get('developer_tools_text_error_generated_extension')
                    . '<br>' . $error;
        ...
        //load tabs controller
        $tabs_obj = $this->dispatch('pages/tool/developer_tools_tabs', ['create']);
        $this->data['dev_tabs'] = $tabs_obj->dispatchGetOutput();
    ...
    public function edit()
    {
        ...
        if ($this->request->is_POST()) {
            ...
            if ($result) {
                ...
                $error = implode('<br>', $this->model_tool_developer_tools->error);
                $this->data['error_warning'] = $this->language->get(
                        'developer_tools_text_error_generated_extension'
                    ) . '<br>' . $error;
                foreach ($this->request->post as $key => $value) {
                    $this->data[$key] = $value;
                }
        ...
        if ($this->_is_open_project()) {
            $project_info = $this->model_tool_developer_tools->getProjectConfig(
                $this->session->data['dev_tools_prj_id']
            );
        } else {
            $project_info = null;
        ...
        $this->data['dt_attention'] = sprintf(
            ...
            DIR_EXT . 'developer_tools'
            . DS . 'projects'
            . DS . 'dev_tools_project_' . $this->session->data['dev_tools_prj_id']
        );
        ...
        $this->data['dev_tabs'] = $tabs_obj->dispatchGetOutput();
        $tabs_obj = $this->dispatch('pages/tool/developer_tools_tabs/prjtabs', ['edit']);
    ...
    protected function _getForm($mode = 'full')
    {
        ...
        $this->data['action'] = $this->html->getSecureURL(
            'tool/developer_tools/' . ($mode == 'short' ? 'create' : 'edit')
        );
        $this->data['heading_title'] = $this->language->get('developer_tools_name');
        $this->data['update'] = '';
        $form = new AForm('ST');
        ...
        $this->data['form']['submit'] = $form->getFieldHtml(
            [
                ...
                'name'  => 'submit',
                'text'  => ($mode == 'short'
                    ? $this->language->get('button_generate')
     ...
     * @throws AException
     */
    protected function _build_common($form, $mode = 'full')
    {
        ...
        $result = $this->model_tool_mp_api->processRequest([]);
        ...
        $this->data['form']['fields']['common']['cart_version'] = [
            $form->getFieldHtml(
                [
                    'type'     => 'input',
                    'name'     => 'cartversions[]',
        ...
        $this->data['form']['fields']['common']['install_sql'] = $form->getFieldHtml(
            [
                ...
                'name'    => 'install_sql',
                'value'   => 1,
                'checked' => (bool)$this->data['install_sql'],
                'style'   => "btn_switch"
            ]);
        if (is_file(DIR_EXT . $this->data['extension_txt_id'] . DS . 'install.sql')) {
            $this->data['form']['fields']['common']['install_sql'] .= ' '
                . $this->language->get('developer_tools_entry_already_created');
        }
        $this->data['form']['fields']['common']['install_php'] = $form->getFieldHtml(
            [
                ...
                'name'    => 'install_php',
                'value'   => 1,
                'checked' => (bool)$this->data['install_php'],
                'style'   => "btn_switch"
            ]);
        if (is_file(DIR_EXT . $this->data['extension_txt_id'] . DS . 'install.php')) {
            $this->data['form']['fields']['common']['install_php']
                .= ' ' . $this->language->get('developer_tools_entry_already_created');
        }
     ...
     * @throws AException
     */
    protected function _build_languages($form)
    {
        //admin
        ...
        if (!(array)$this->data['languages']['admin']) {
            $admin_languages = ['english' => 'english'];
        } else {
        ...
        //storefront
        $options = array_column($this->data['all_languages'], 'name', 'directory');
     ...
     * @throws AException
     */
    protected function _build_language_settings($form)
    {

        $this->data['form']['fields']['language_extension_settings']['language_extension_name'] = $form->getFieldHtml(
            [
                'type'     => 'input',
        ...
        $this->data['form']['fields']['language_extension_settings']['language_extension_code'] = $form->getFieldHtml(
            [
                'type'     => 'input',
        ...
        $this->data['form']['fields']['language_extension_settings']['language_extension_locale'] = $form->getFieldHtml(
            [
                'type'     => 'input',
        ...
        $this->data['form']['fields']['language_extension_settings']['language_extension_directory'] = $form->getFieldHtml(
            [
                'type'     => 'input',
                'name'     => 'language_extension_directory',
                'value'    => $this->data['language_extension_directory'],
                'required' => true,
                'help_url' => 'https://docs.abantecart.com/pages/localization/languages/edit_language.html',
            ]
        );

        $this->data['form']['fields']['language_extension_settings']['language_extension_flag_icon'] = $form->getFieldHtml(
            [
                'type'  => 'file',
        ...
        $value = $this->data['language_extension_direction'] ?: 'ltr';
        $this->data['form']['fields']['language_extension_settings']['language_extension_direction'] = $form->getFieldHtml(
            [
                'type'        => 'input',
        ...
        $value = $this->data['language_extension_date_format_short'] ?: 'm/d/Y';
        $this->data['form']['fields']['language_extension_settings']['language_extension_date_format_short'] = $form->getFieldHtml(
            [
                'type'        => 'input',
                ...
                'help_url'    => 'https://php.net/manual/en/function.date.php',
            ]
        );

        $value = $this->data['language_extension_date_format_long'] ?: 'l dS F Y';
        $this->data['form']['fields']['language_extension_settings']['language_extension_date_format_long'] = $form->getFieldHtml(
            [
                'type'        => 'input',
        ...
        $value = $this->data['language_extension_time_format_short'] ?: 'H:i';
        $this->data['form']['fields']['language_extension_settings']['language_extension_time_format_short'] = $form->getFieldHtml(
            [
                'type'        => 'input',
        ...
        $value = $this->data['language_extension_time_format'] ?: 'h:i:s A';
        $this->data['form']['fields']['language_extension_settings']['language_extension_time_format'] = $form->getFieldHtml(
            [
                'type'        => 'input',
        ...
        $value = $this->data['language_extension_decimal_point'] ?: '.';
        $this->data['form']['fields']['language_extension_settings']['language_extension_decimal_point'] = $form->getFieldHtml(
            [
                'type'        => 'input',
                ...
                'help_url'    => 'https://php.net/manual/ru/function.number-format.php',
            ]
        );

        $value = $this->data['language_extension_thousand_point'] ?: ',';
        $this->data['form']['fields']['language_extension_settings']['language_extension_thousand_point'] = $form->getFieldHtml(
            [
                'type'        => 'input',
        ...
        if ($this->_is_open_project()) {
            $all_languages = array_column($this->language->getAvailableLanguages(), 'name', 'language_id');
            $this->data['form']['fields']['language_extension_settings']['source_language'] = $form->getFieldHtml(
                [
                    'type'    => 'selectbox',
            ...
            $this->data['form']['fields']['language_extension_settings']['translation_method'] = $form->getFieldHtml(
                [
                    'type'    => 'selectbox',
            ...
            $this->data['form']['language_extension_settings']['note']
                = $this->language->get('developer_tools_translation_note');
        }
    }
     ...
     */
    protected function _build_admin($form)
    {
        $cTypes = [
            'page'     => 'pages',
            'response' => 'responses',
        ...
        foreach ($admin_controllers as $c) {
            ...
            foreach ($cTypes as $alias => $controllerType) {
                if (str_starts_with($c['route'], $controllerType . '/')) {
                    $ac[$alias][] = [
                        'route' => str_replace($controllerType . '/', '', $c['route']),
                        'file'  => $c['file']
                    ];
                }
            }
        ...
        $this->data['admin_page_controller_routes'][''] = [];
        ...
        $add_button = $this->dispatch('responses/tool/developer_tools/addbutton', ['id' => 'admin_page_controllers']);
        ...
        $this->data['admin_response_controller_routes'][''] = [];
        ...
        $add_button = $this->dispatch('responses/tool/developer_tools/addbutton', ['id' => 'admin_response_controllers']);
        ...
        $this->data['admin_api_controller_routes'][''] = [];
        ...
        $add_button = $this->dispatch('responses/tool/developer_tools/addbutton', ['id' => 'admin_api_controllers']);
        ...
        $this->data['admin_task_controller_routes'][''] = [];
        ...
        $add_button = $this->dispatch('responses/tool/developer_tools/addbutton', ['id' => 'admin_task_controllers']);
        ...
        $add_button = $this->dispatch('responses/tool/developer_tools/addbutton', ['id' => 'admin_models']);
        ...
        foreach ($admin_views as $c) {
            if (str_starts_with($c['route'], 'pages/')) {
                $av['page'][] = [
                    'route' => str_replace('pages/', '', $c['route']),
                    'file'  => $c['file']
                ];
            } elseif (str_starts_with($c['route'], 'responses/')) {
                $av['response'][] = [
        ...
        $this->data['admin_page_view_routes'][''] = [];
        ...
        $add_button = $this->dispatch('responses/tool/developer_tools/addbutton', ['id' => 'admin_page_views']);
        ...
        $this->data['admin_response_view_routes'][''] = [];
        ...
        $add_button = $this->dispatch('responses/tool/developer_tools/addbutton', ['id' => 'admin_response_views']);
     ...
     * @throws ReflectionException
     */
    protected function _build_storefront($form)
    {
        $cTypes = [
            'page'     => 'pages',
            'response' => 'responses',
        ...
        $this->data['storefront_page_controller_routes'][''] = [];
        ...
        $add_button = $this->dispatch('responses/tool/developer_tools/addbutton', ['id' => 'storefront_page_controllers']);
        ...
        $this->data['storefront_response_controller_routes'][''] = [];
        ...
        $add_button = $this->dispatch('responses/tool/developer_tools/addbutton', ['id' => 'storefront_response_controllers']);
        ...
        $this->data['storefront_api_controller_routes'][''] = [];
        ...
        $add_button = $this->dispatch('responses/tool/developer_tools/addbutton', ['id' => 'storefront_api_controllers']);
        ...
        $this->data['storefront_block_controller_routes'][''] = [];
        ...
        $add_button = $this->dispatch('responses/tool/developer_tools/addbutton', ['id' => 'storefront_block_controllers']);
...
// storefront models
        ...
        $this->data['storefront_model_routes'][''] = [];
        ...
        $add_button = $this->dispatch('responses/tool/developer_tools/addbutton', ['id' => 'storefront_models']);
        ...
        $this->data['storefront_page_view_routes'][''] = [];
        ...
        $add_button = $this->dispatch('responses/tool/developer_tools/addbutton', ['id' => 'storefront_page_views']);
        ...
        $this->data['storefront_response_view_routes'][''] = [];
        ...
        $add_button = $this->dispatch('responses/tool/developer_tools/addbutton', ['id' => 'storefront_response_views']);
    ...
    protected function _validate_create_data($data)
    {
        ...
        if ($data['extension_type'] == 'language') {
            if (str_contains($data['extension_txt_id'], 'language')) {
            ...
            if (!$data['language_extension_direction']
                || !in_array(strtolower($data['language_extension_direction']), ['rtl', 'ltr'])
            ) {
            ...
            foreach ($fields as $field_name) {
                if (!$data['language_extension_' . $field_name]) {
                    $this->error['language_extension_' . $field_name] = 'Please Fill '
                        . $this->language->get('developer_tools_entry_language_extension_' . $field_name) . '!';
                }
            }
    ...
    public function package()
    {
        ...
        $this->data['heading_title'] = $this->language->get('developer_tools_name')
            . ' - ' . $this->language->get('developer_tools_tab_generate_package');
        ...
        if ($this->request->is_POST()) {
            ...
            if (!$result || !is_file($result) || !is_readable($result)) {
                ...
                $this->data['error_warning'] = 'Error: Something went wrong. Please check directory '
                    . DIR_APP_SECTION . 'system' . DS . 'temp for files and error log.';
        ...
        $tabs_obj = $this->dispatch('pages/tool/developer_tools_tabs', ['project']);
        $this->data['dev_tabs'] = $tabs_obj->dispatchGetOutput();
        $tabs_obj = $this->dispatch('pages/tool/developer_tools_tabs/prjtabs', ['package']);
        $this->data['prj_tabs'] = $tabs_obj->dispatchGetOutput();
    ...
    public function cloneTemplate()
    {
        ...
        if ($this->request->is_POST()) {
            ...
            if ($result) {
                ...
                if (has_value($this->request->post['clone_method'])) {
                    $url = $this->html->getSecureURL(
                        'setting/setting',
                        '&active=appearance&tmpl_id=' . $data['extension_txt_id']
                    );
                } else {
                    $url = $this->html->getSecureURL('tool/developer_tools/edit');
                }
                ...
                $this->view->assign(
                    'error_warning',
                    $this->language->get('developer_tools_text_error_generated_extension') . '<br>' . $error
                );
                foreach ($this->request->post as $key => $value) {
                    $this->data[$key] = $value;
                }
        ...
        //load tabs controller
        $tabs_obj = $this->dispatch('pages/tool/developer_tools_tabs', ['clone']);
        $this->data['dev_tabs'] = $tabs_obj->dispatchGetOutput();
    ...
    public function removeCoreTemplate()
    {
        ...
        if (!$tmpl_id
            || !is_dir(DIR_STOREFRONT . 'view' . DS . $tmpl_id)
            || $tmpl_id == 'default'
        ) {
            redirect($url);
        }
        ...
        if ($result) {
            ...
            $error = implode('<br>', $this->model_tool_developer_tools->error);
            $this->session->data['error'] = $this->language->get('developer_tools_text_error_generated_extension')
                . '<br>' . $error;
        }
    ...
    protected function _getCloneForm()
    {
        $this->view->assign('heading_title', $this->language->get('developer_tools_name'));
        $this->document->initBreadcrumb(
            [
     ...
     * @throws AException
     */
    protected function _check4NewTemplates($data)
    {
        //check is templates already in main.php file of extension
        if ($this->_is_open_project() && has_value($data['templates'])) {
            ...
            foreach ($data['templates'] as $template) {
                ...
                if (!$found) {
                    $prj_config['views']['storefront'][] = ['route' => $route, 'file' => $file];
                }
            }
    ...
    protected function _getBlockForm()
    {
        ...
        if (!$block_id) {
            ...
            $this->data ['action'] = $this->html->getSecureURL(
                'tool/developer_tools/edit_block',
                '&block_id=' . $block_id
            );
            $this->data ['form_title'] = $this->language->get('developer_tools_text_edit_block')
                . ' ' . $this->data['name'];
            $this->data ['update'] = '';
            $form = new AForm ('ST');
        }
        ...
        $form->setForm(['form_name' => 'GenericBlockFrm', 'update' => $this->data ['update']]);
        ...
        $this->data['form']['fields']['block_txt_id'] = $form->getFieldHtml([
            'type'     => 'input',
            'name'     => 'block_txt_id',
        ...
        $this->data['form']['fields']['block_controller'] = $form->getFieldHtml(
            [
                'type'     => 'input',
                'name'     => 'controller',
                'required' => true,
        ...
        if ($this->_is_open_project()) {
            ...
            $tpl_path = DIR_EXT . $extension_id . DS . 'storefront' . DS . 'view' . DS . 'my_template' . DS . 'template' . DS . '';
    ...
    protected function _validateBlockForm()
    {
        ...
        $required = ['block_txt_id', 'controller'];
